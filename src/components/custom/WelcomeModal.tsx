...
const WelcomeModal = forwardRef<WelcomeModalHandle>((props, ref) => {
  ...
  const emailAddress = "123@qq.com";
  const siteUrl = "https://cckiller.com";
  // B站视频嵌入地址
  ...
  useEffect(() => {
    // 检查是否是首次访问
    const hasVisited = localStorage.getItem('cckiller_welcome_shown');
    if (!hasVisited) {
      // 延迟 1 秒显示，让页面先加载
      setTimeout(() => {
        setModalState('expanded');
      }, 1000);
      localStorage.setItem('cckiller_welcome_shown', 'true');
    }
  }, []);
